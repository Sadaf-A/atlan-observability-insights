...
export default function TraceViewer() {
  ...
  useEffect(() => {
    axios
      .get("https://atlan345678.as.r.appspot.com/api/urls", {
        headers: {
  ...
  const fetchTraces = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get("https://atlan345678.as.r.appspot.com/api/traces", {
        params: { monitoredUrl },
      });
      setTraces(data);
    } catch (error) {
  ...
  const runDiagnosis = async () => {
    ...
    try {
      const { data } = await axios.post("https://atlan345678.as.r.appspot.com/api/diagnose", {
        monitoredUrl,
      });
  ...
  const filteredTraces = traces.slice(-20).filter((trace) => {
    const matchesStatus =
      filterStatus === null ||
      trace.spans.some((span) => span.tags.statusCode === filterStatus);
    const matchesSearch =
      searchQuery === "" ||
    ...
    All Traces
            ...
            <button
              style={{
                ...
                alignItems: "center",
                backgroundColor:
                  filterStatus === 200 ? currentTheme.success : "transparent",
                color: filterStatus === 200 ? "#FFFFFF" : currentTheme.success,
                border:
                  filterStatus === 200
                    ? "none"
                    : `1px solid ${currentTheme.success}`,
              }}
            ...
            <button
              style={{
                ...
                alignItems: "center",
                backgroundColor:
                  filterStatus === 500 ? currentTheme.error : "transparent",
                color: filterStatus === 500 ? "#FFFFFF" : currentTheme.error,
                border:
                  filterStatus === 500
                    ? "none"
                    : `1px solid ${currentTheme.error}`,
              }}
              ...
              {filteredTraces.length === 0 ? (
                ...
                >
                  ...
                  <p style={{ fontSize: "14px" }}>
                    Try changing your search criteria or refreshing the data
                  </p>
                </div>
              ) : (
                <div
              ...
              <div style={{ marginTop: "32px" }}>
                ...
                >
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart
                      data={filteredTraces.flatMap((trace) =>
                        trace.spans.map((span) => ({
