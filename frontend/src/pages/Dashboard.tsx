...
const Dashboard: React.FC = () => {
  const [dbMetrics, setDbMetrics] = useState<Metric[]>([]);
  ...
  const handleSetUrl = async () => {
    ...
    try {
      ...
      const structuredEndpoints: Record<string, any> = {
        GET: [],
        POST: [],
        PUT: [],
        DELETE: [],
  ...
  const dbLatencyChartData = dbMetrics
    .map((metric) => ({
      name: metric.timestamp
        ? new Date(metric.timestamp).toLocaleTimeString()
        : "",
  ...
  const connectWebSocket = useCallback(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data.dbMetrics && Array.isArray(data.dbMetrics)) {
          setDbMetrics((prevMetrics) => [...data.dbMetrics]);
        }
