...
const Dashboard: React.FC = () => {
    ...
    const [endpoints, setEndpoints] = useState<Endpoint[]>([{ method: "GET", path: "" }]);
    ...
    const handleSetUrl = async () => {
        ...
        try {
            ...
            const structuredEndpoints: Record<string, string[]> = { GET: [], POST: [], PUT: [], DELETE: [], PATCH: [] };
    
            endpoints.forEach(({ method, path }) => {               
                structuredEndpoints[method as keyof typeof structuredEndpoints]?.push(path);
            });
    
            localStorage.setItem("monitoredUrls", JSON.stringify({ baseUrl: inputUrl, endpoints: structuredEndpoints }));
                ...
                {index === endpoints.length - 1 && (
                        ...
                        <div>
                            <div style={{
                                ...
                                <p style={{
                                    ...
                                    Monitoring: <span style={{ color: currentTheme.primary }}>{monitoredUrl}</span>
                                </p>
                                <button 
                                    style={{
                                        padding: "8px 16px",
                            ...
                            <div style={{
                                ...
                                <div style={{
                                    ...
                                    <h3 style={{
                                        ...
                                        marginBottom: "16px",
                                        color: currentTheme.text
                                    }}>Latency Over Time</h3>
                                    <ResponsiveContainer width="100%" height={300}>
                                        <LineChart data={latencyChartData}>
                                            <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? "rgba(244, 245, 247, 0.2)" : "rgba(23, 43, 77, 0.2)"} />
                                            <XAxis 
                                                dataKey="name" 
                                            ...
                                            <YAxis 
                                                stroke={darkMode ? "rgba(244, 245, 247, 0.6)" : "rgba(23, 43, 77, 0.6)"}
                                                label={{ value: 'Latency (ms)', angle: -90, fill: currentTheme.text }}
                                            />
                                            <Tooltip 
                                                contentStyle={{
                                                    backgroundColor: currentTheme.cardBackground,
                                ...
                                <div style={{
                                    ...
                                    <ResponsiveContainer width="100%" height={300}>
                                        <BarChart data={averageLatencyChartData}>
                                            ...
                                            <YAxis 
                                                stroke={darkMode ? "rgba(244, 245, 247, 0.6)" : "rgba(23, 43, 77, 0.6)"}
                                                label={{ value: 'Average Latency (ms)', angle: -90, fill: currentTheme.text }}
                                            />
                                            <Tooltip 
                                                contentStyle={{
                                                    backgroundColor: currentTheme.cardBackground,
                    ...
                    {!isMonitoring && (
                        <div style={{
                            ...
                            <p style={{ fontSize: "16px", marginBottom: "8px" }}>Enter a URL to start monitoring</p>
                        </div>
                    )}
                </div>
            </div>
