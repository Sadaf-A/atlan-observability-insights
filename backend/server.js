...
app.get("/api/traces", async (req, res) => {
  try {
    ...
    const traces = await Trace.find({
      "spans.operationName": new RegExp(monitoredUrl, "i"),
    }).sort({ createdAt: -1 });
    res.json(traces);
  } catch (error) {
...
app.post("/api/set-url", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!baseUrl || !endpoints) {
      ...
      return res
        .status(400)
        .json({ error: "Base URL and endpoints are required." });
    }
    ...
    res
      .status(201)
      .json({ message: `Monitoring ${baseUrl}`, baseUrl, endpoints });
  } catch (error) {
    span.recordException(error);
    console.log(error);
...
app.post("/api/diagnose", async (req, res) => {
  try {
    ...
    const aiResponse =
      response.data?.message?.content[0]?.text || "No response from AI";
...
app.get("/api/seed", async (req, res) => {
  ...
  await Performance.insertMany([
    {
      timestamp: new Date("2025-03-22T12:00:00Z"),
      avgResponseTime: 120,
      errorRate: 0.05,
    },
    {
      timestamp: new Date("2025-03-23T12:00:00Z"),
      avgResponseTime: 135,
      errorRate: 0.07,
    },
    {
